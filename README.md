...
5) Once you are in the ddos-detection directory , enter the following command: ls
6) This lists all the files and directories present in the current folder.
...
15) If any other error regrading libraries pop up , just use pip install <library_name> where <library_name> is the name of the library to be installed.
16) After successfully running the program , the accuracy is provided for every 50 packets sent to the server.It also provides the Entropy value and outputs whether the DDoS attack has been carried out or not.
